...
The accepted path uses external module
`46slv/zmk-input-processor-roba-scroll`, pinned in `config/west.yml` and
documented in `docs/ROBA_SCROLL_MODULE.md`.
...
Do not restore the old snap/inertia modules or a downstream
`zip_scroll_scaler`. The unified processor owns retained axis
