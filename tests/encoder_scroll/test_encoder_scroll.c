...
static void test_fast_guard_and_max_arm(void) {
    ...
    for (size_t i = 0; i < sizeof(times) / sizeof(times[0]); i++) {
        ...
        assert(result.inertia_armed == (i == 2));
    }
}

static void test_four_x_responds_after_one_matching_interval(void) {
    struct roba_encoder_scroll_state state = fresh_state();
    assert(roba_encoder_scroll_feed(&state, &config, 1, 1000).multiplier == 1);
    assert(roba_encoder_scroll_feed(&state, &config, 1, 1120).multiplier == 4);
}
...
int main(void) {
    ...
    test_four_x_responds_after_one_matching_interval();
