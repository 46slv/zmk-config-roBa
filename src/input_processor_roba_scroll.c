 ...
 * roBa changes integrate retained axis selection and active scaling.
 * SPDX-License-Identifier: MIT
 */

#define DT_DRV_COMPAT zmk_input_processor_roba_scroll
...
#error "zmk,input-processor-roba-scroll: central role required. " \
...
/* struct scroll_inertia_config lives in roba_scroll_math.h so
...
struct scroll_inertia_data {
    ...
    struct roba_axis_lock_state axis_lock;
    bool axis_free_active;

    /* Active input uses the same scale as coast. Remainders are kept
     * separately per HID wheel axis so small deltas are delayed, not lost. */
    int64_t active_remainder_x;
    int64_t active_remainder_y;
...
/* Same-direction coasting policy (bump / absorb / weak-pass).  Shared
 * by the single-axis and AXIS_BOTH COASTING handlers: both need the
 * same three-way decision on a same-direction event, and the only
 * per-handler difference is what runs *before* (cross-axis accumulator
 * clear in single-axis) and *after* (fall-through behaviour on a
 * reversal).
 *
 * `axis_vel` points at the coasting component to mutate on a bump.
 * `compare_vel` is the *reference coast speed* the event is compared
 * against — |coast| for bumping, |coast|/2 for the clearly-weaker
 ...
 * "freeze" direct input because the per-axis half threshold was too
 * low to clear).
 *
 ...
 *       the current coast — this is almost certainly the tail of the
 *       ball's natural post-flick deceleration (which decays more
 *       slowly than the configured inertia, so its events stay within
 *       ~50% of compare_vel as the bump path tracks it).  Absorb to
 *       avoid double-emitting the same physical motion that inertia
 *       is already representing.
 *   (2) Post-handoff AND event velocity is clearly below half of the
 *       coast — a new weak user gesture, not ball tail.  Pass through
 *       so the user's own input drives visible scroll alongside the
 *       decaying tick; silently absorbing it would stall the screen
 *       at the late-coast sparse-emit regime.  We do not cancel the
 *       coast: the event is much weaker than the current inertia, so
 *       letting inertia finish out is natural. */
static bool coast_same_dir(struct scroll_inertia_data *data,
                           const struct scroll_inertia_config *cfg,
                           struct input_event *event,
                           int32_t *axis_vel,
                           int32_t compare_vel) {
    ...
    bool clearly_weaker = abs32(event_vel_fp) < (abs32(compare_vel) >> 1);
...
static bool handle_tracked_in_coasting(struct scroll_inertia_data *data,
    ...
    bool same_dir = *axis_vel != 0 &&
                    (event->value > 0) == (*axis_vel > 0);
    ...
    if (same_dir) {
        ...
        data->cross_axis_accumulated = 0;
        /* Single-axis: *axis_vel already holds the vector magnitude
         * (to_coasting folds it in), so per-axis and magnitude-based
         * comparison are equivalent here. */
        return coast_same_dir(data, cfg, event, axis_vel, *axis_vel);
    }
 ...
 *
 * Same-direction bump / absorb / weak-passthrough goes through the
 * shared coast_same_dir helper. */
static bool handle_tracked_in_coasting_both(struct scroll_inertia_data *data,
    ...
    if (same_dir) {
        /* AXIS_BOTH: pass the vector magnitude as the comparison
         * reference so the "clearly weaker" gate (event < coast/2)
         * fires at the same physical speed as in single-axis mode.
         * Using *axis_vel alone would shrink the threshold to the
         * per-axis component, making a diagonal coast absorb weak
     ...
     *
     * suppress_count is cleared too: a reversal on one axis is a
     * deliberate direction change in this gesture, so the same-dir
     * absorption count from before the reversal no longer represents
     * the user "rolling past a stale coast".  Leaving it accumulating
     * means an otherwise-healthy mixed-direction pan could hit
     * suppress-limit sooner than intended. */
    *axis_vel   = 0;
    *axis_peak  = 0;
    *axis_accum = 0;
    data->suppress_count = 0;
...
static bool filter_and_retain_axis(struct scroll_inertia_data *data,
                                   const struct scroll_inertia_config *cfg,
                                   struct input_event *event,
                                   int64_t now) {
    ...
    enum roba_axis_direction event_axis = event->code == INPUT_REL_HWHEEL
        ? ROBA_AXIS_X
        : ROBA_AXIS_Y;
    if ((event_axis == ROBA_AXIS_X && cfg->invert_x) ||
        (event_axis == ROBA_AXIS_Y && cfg->invert_y)) {
        event->value = event->value == INT32_MIN ? INT32_MAX : -event->value;
    }
    struct roba_axis_lock_config axis_cfg = cfg->axis_lock;
    if (unlock_active(cfg)) axis_cfg.mode = 1;
    bool free_mode = axis_cfg.mode == 1;
    if (free_mode != data->axis_free_active) {
        to_idle(data);
        roba_axis_lock_reset(&data->axis_lock);
        data->active_remainder_x = 0;
        data->active_remainder_y = 0;
        data->axis_free_active = free_mode;
    }
    struct roba_axis_lock_result result = roba_axis_lock_feed(
        &data->axis_lock, &axis_cfg, event_axis, event->value, now);

    if (result.action == ROBA_AXIS_WAIT) {
        event->value = 0;
        event->sync = false;
        return false;
    }

    if (result.direction_changed && !free_mode) {
        /* A new selected axis is a new gesture. Cancel a previous coast and
         * clear the old axis velocity before feeding the retained delta. */
        to_idle(data);
        data->active_remainder_x = 0;
        data->active_remainder_y = 0;
    }

    event->code = result.direction == ROBA_AXIS_X
        ? INPUT_REL_HWHEEL
        : INPUT_REL_WHEEL;
...
static int32_t scale_active_event(struct scroll_inertia_data *data,
                                  const struct scroll_inertia_config *cfg,
                                  uint16_t code,
                                  int32_t value) {
    int64_t *remainder = code == INPUT_REL_HWHEEL
        ? &data->active_remainder_x
        : &data->active_remainder_y;
    return scale_active_with_remainder(value, remainder, cfg);
}
...
static void inertia_tick_handler(struct k_work *work) {
    ...
    if (emit_x != 0 || emit_y != 0) {
        ...
        if (cfg->axis_lock.mode == 0 && !unlock_active(cfg) && swap_active(cfg)) {
            out_x = emit_y;
            out_y = emit_x;
        }
...
static int scroll_inertia_handle_event(const struct device *dev,
    ...
    if (!filter_and_retain_axis(data, cfg, event, now)) {
    ...
    if (pass_through && cfg->axis_lock.mode == 0 &&
        !unlock_active(cfg) && swap_active(cfg)) {
    ...
    if (pass_through) {
        event->value = scale_active_event(data, cfg, event->code, event->value);
    }
...
#define SCROLL_INERTIA_INST(n)                                                \
    ...
    static const struct scroll_inertia_config scroll_inertia_config_##n = {   \
        ...
        .axis_lock = {                                                        \
            .x_ratio_num = DT_INST_PROP_BY_IDX(n, snap_x_ratio, 0),           \
            .x_ratio_den = DT_INST_PROP_BY_IDX(n, snap_x_ratio, 1),           \
            .y_ratio_num = DT_INST_PROP_BY_IDX(n, snap_y_ratio, 0),           \
            .y_ratio_den = DT_INST_PROP_BY_IDX(n, snap_y_ratio, 1),           \
            .require_events = DT_INST_PROP(n, snap_events),                   \
            .immediate_threshold = DT_INST_PROP(n, snap_immediate),           \
            .lock_ms = DT_INST_PROP(n, snap_lock_ms),                         \
            .lock_events = DT_INST_PROP(n, snap_lock_events),                 \
            .idle_reset_ms = DT_INST_PROP(n, snap_idle_ms),                   \
            .switch_threshold = DT_INST_PROP(n, snap_switch),                 \
            .mode = DT_INST_PROP(n, axis_mode),                               \
        },                                                                    \
    };                                                                        \
...
#define SCROLL_INERTIA_LAYER_OFF_RESET(n)                                     \
    if (scroll_inertia_config_##n.layer == ev->layer) {                       \
        ...
        roba_axis_lock_reset(&scroll_inertia_data_##n.axis_lock);             \
        scroll_inertia_data_##n.axis_free_active = false;                     \
        scroll_inertia_data_##n.active_remainder_x = 0;                       \
        scroll_inertia_data_##n.active_remainder_y = 0;                       \
    }

#define ROBA_SCROLL_RESET_ALL(n)                                              \
    to_idle(&scroll_inertia_data_##n);                                        \
    roba_axis_lock_reset(&scroll_inertia_data_##n.axis_lock);                 \
    scroll_inertia_data_##n.axis_free_active = false;                         \
    scroll_inertia_data_##n.active_remainder_x = 0;                           \
...
static int scroll_inertia_layer_cb(const zmk_event_t *eh) {
    ...
    if (ev == NULL || ev->state) {
        if (ev == NULL && as_zmk_endpoint_changed(eh) != NULL) {
            DT_INST_FOREACH_STATUS_OKAY(ROBA_SCROLL_RESET_ALL)
        }
...
ZMK_LISTENER(roba_scroll, scroll_inertia_layer_cb);
ZMK_SUBSCRIPTION(roba_scroll, zmk_layer_state_changed);
ZMK_SUBSCRIPTION(roba_scroll, zmk_endpoint_changed);
