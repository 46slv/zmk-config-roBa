...
#ifndef ZMK_INPUT_PROCESSOR_ROBA_SCROLL_MATH_H
#define ZMK_INPUT_PROCESSOR_ROBA_SCROLL_MATH_H

#include <stdbool.h>
#include <stdint.h>

#include "roba_axis_lock.h"
...
struct scroll_inertia_config {
    ...
    struct roba_axis_lock_config axis_lock;
};
...
/* Scale an active integer event while retaining the fractional numerator.
 * This is the active-path counterpart of accumulate_and_emit used by coast. */
static inline int32_t scale_active_with_remainder(
    int32_t value, int64_t *remainder,
    const struct scroll_inertia_config *cfg) {
    int32_t divisor = safe_scale_div(cfg->scale_div);
    int64_t product = (int64_t)value * cfg->scale;
    if (product > 0 && *remainder > INT64_MAX - product) {
        *remainder = INT64_MAX;
    } else if (product < 0 && *remainder < INT64_MIN - product) {
        *remainder = INT64_MIN;
    } else {
        *remainder += product;
    }
    int64_t output = *remainder / divisor;
...
#endif /* ZMK_INPUT_PROCESSOR_ROBA_SCROLL_MATH_H */
